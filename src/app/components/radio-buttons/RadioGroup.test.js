...
test("Radio button matches stored snapshot", () => {
    const component = renderer.create(<RadioGroup {...defaultProps} />);
    expect(component.toJSON()).toMatchSnapshot();
});

test("Handle change passes correct value caller onChange method", () => {
    let passedToParent = {};
    const mockedOnChange = event => {
        passedToParent = event;
    };
    const component = shallow(<RadioGroup {...defaultProps} checked={false} onChange={mockedOnChange} />);
