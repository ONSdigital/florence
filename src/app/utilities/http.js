...
export default class http {
     ...
     * @param {boolean} callerHandles401 - Flag to decide whether caller or global handler is to handle 401 responses
     * @param {boolean} returnResponseHeaders - returns headers as well as body
     * @param {boolean} multipartFormData - multipart header
     *
     * @returns {Promise} which returns the response body in JSON format
     */
    static post(uri, body, multipartFormData, willRetry, callerHandles401, returnResponseHeaders) {
        return request("POST", uri, willRetry, undefined, body, callerHandles401, returnResponseHeaders, multipartFormData);
    }
