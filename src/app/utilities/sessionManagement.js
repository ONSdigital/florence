...
export default class sessionManagement {
  ...
  static setSessionExpiryTime = (sessionExpiryTime, refreshExpiryTime) => {
    /* Set expiry and refresh time within localStorage under ons_auth_state */
    /* Keeping sessionStorage as back-up */
    if (sessionExpiryTime != null) {
      if (localStorage.getItem("ons_auth_state")) {
        const store = JSON.parse(localStorage.getItem("ons_auth_state"));
        store["session_expiry_time"] = sessionExpiryTime;
        localStorage.setItem("ons_auth_state", JSON.stringify(store));
      } else {
    ...
    if (refreshExpiryTime != null) {
      const store = JSON.parse(localStorage.getItem("ons_auth_state"));
      store["refresh_expiry_time"] = refreshExpiryTime;
      localStorage.setItem("ons_auth_state", JSON.stringify(store));
  ...
  static removeTimers = () => {
    localStorage.removeItem("ons_auth_state");
  ...
  static initialiseSessionExpiryTimers = () => {
    const sessionExpiryTime =
      JSON.parse(localStorage.getItem("ons_auth_state")).session_expiry_time ||
      sessionStorage.getItem("session_expiry_time");
    const refreshExpiryTime =
      JSON.parse(localStorage.getItem("ons_auth_state")).refresh_expiry_time ||
      sessionStorage.getItem("refresh_expiry_time");
    if (sessionExpiryTime != null) {
      ...
      this.startExpiryTimer(
        "sessionTimerInvasive",
        sessionExpiryTime,
        this.timeOffsets.invasiveRenewal,
        this.warnSessionSoonExpire
      );
    }
    if (refreshExpiryTime != null) {
      ...
      this.startExpiryTimer(
        "refreshTimerInvasive",
        refreshExpiryTime,
        this.timeOffsets.invasiveRenewal,
        this.warnRefreshSoonExpire
      );
    }
  };

  static startExpiryTimer = (
    name,
    expiryTime,
    ...
    if (expiryTime != null) {
      ...
      const timerInterval =
        expireTimeInUTCDate - offsetInMilliseconds - nowInUTC;
  ...
  static removeWarnings = () => {
    store.dispatch(
      removePopouts(["session-expire-soon", "refresh-expire-soon"])
    ); // Todo all
  };
  ...
  static refreshSession = () => {
    ...
    const renewError = (error) => {
      console.error(
        "an unexpected error has occurred when extending the users session"
      );
      if (error != null) {
        console.error(error);
      }
