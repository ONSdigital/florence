...
test("Dataset details page matches stored snapshot", () => {
    const component = renderer.create(<DatasetMetadata {...defaultProps} />);
...
test("Changing an input value updates the state to show a change has been made", async () => {
    const asyncComponent = await shallow(<DatasetMetadata {...defaultProps} />);
    const event = {
        ...mockEvent,
        target: {
...
describe("Component's state maps to API request correctly when", () => {
    ...
    it("related links data has been updated", () => {
        ...
        expect(defaultComponent.instance().mapStateToAPIRequest().related_datasets[0]).toEqual(
            expect.objectContaining(mockLinksState.relatedLinks[0])
        );
    });
    ...
    it("related methodlogies data has been updated", () => {
        ...
        defaultComponent.setState(mockState);
        expect(defaultComponent.instance().mapStateToAPIRequest().methodologies[0]).toEqual(
            expect.objectContaining(mockState.relatedMethodologies[0])
        );
    });
    ...
    it("keywords data has been updated", () => {
        ...
        expect(defaultComponent.instance().mapStateToAPIRequest().keywords).toEqual(
            expect.arrayContaining(["foo", "bar", "more", "two words", "another"])
        );
    });
...
describe("Saving changes and submitting for review/approval", () => {
    it("passes any errors from updating review state to the error handler", async () => {
        collections.setDatasetStatusToComplete.mockImplementationOnce(() => Promise.reject({ status: 500, statusText: "Mocked error" }));
    ...
    it("passes any errors from metadata updates to the error handler", async () => {
        datasets.updateDatasetMetadata.mockImplementationOnce(() => Promise.reject({ status: 500, statusText: "Mocked error" }));
        await defaultComponent.instance().handleSave(mockEvent, true, false);
        const handlerArguments = handleMetadataSaveErrors.mock.calls[handleMetadataSaveErrors.mock.calls.length - 1];
