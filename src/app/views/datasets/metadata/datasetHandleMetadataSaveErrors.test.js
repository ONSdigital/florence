...
describe("Errors when updating the review state or saving metadata updates", () => {
    ...
    it("add the correct notification when both requests fail for the same reason", () => {
        notifications.add.mockReset();
        const expectedErrMsg = "Unable to submit for review and save your metadata updates due to a network issue. Please check your internet connection and try again";
    ...
    it("add the correct notifications when both requests fail for different reasons", () => {
        ...
        expect(metadataErrorMsg).toBe("Unable to save your metadata updates due to a network issue. Please check your internet connection and try again");
    ...
    it("add the correct notification when one/both requests return a 400", () => {
        ...
        expect(notifications.add.mock.calls.length).toBe(1);
        expect(errorMsg).toBe("Unable to submit for review and save your metadata updates due to invalid values being submitted. Please check your updates for any issues and try again");

        handleMetadataSaveErrors(errors.badRequest, {}, true, false);
        errorMsg = notifications.add.mock.calls[1][0].message;
        expect(notifications.add.mock.calls.length).toBe(2);
        expect(errorMsg).toBe("Unable to save your metadata updates due to invalid values being submitted. Please check your updates for any issues and try again");
        ...
        expect(errorMsg).toBe("Unable to submit for approval due to invalid values being submitted. Please check your updates for any issues and try again");
    });
    ...
    it("add the correct notification when one/both requests return a 404", () => {
        ...
        let errorMsg = notifications.add.mock.calls[0][0].message;
        expect(notifications.add.mock.calls.length).toBe(1);
        expect(errorMsg).toBe("Unable to submit for approval and save your metadata updates because this collection (my-collection-12345) and dataset couldn't be found");
    ...
    it("add the correct notification when one/both requests return a network error", () => {
        ...
        let errorMsg = notifications.add.mock.calls[0][0].message;
        expect(notifications.add.mock.calls.length).toBe(1);
        expect(errorMsg).toBe("Unable to submit for review and save your metadata updates due to a network issue. Please check your internet connection and try again");
