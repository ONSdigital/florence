            ...
            if (instance !== undefined) {
                return (
                    <div key={i} className="grid simple-table__row">
                        ...
                        <div className="grid__col-3">{instance.status}</div>
                        <div className="grid__col-3">{dateFormat(instance.date, "dd/mm/yy HH:MM")}</div>
                        <div className="grid__col-2">{instance.edition}</div>
                        <div className="grid__col-1">{instance.version}</div>
                    </div>
                );
            }
        });
    ...
    render() {
        return (
            <div className="selectable-table">
                <div className="selectable-table__heading">Title</div>
                {this.props.values.map((item, index) => {
                    return (
                        <details key={index} className="selectable-table__details">
                            ...
                            <div>
                                <Link className="inline-block margin-bottom--1" to={item.datasetURL}>
                                    {" "}
                                ...
                                {item.instances.length > 0 && (
                                    <div>
                                        <div className="grid simple-table__heading">
                                            <div className="grid__col-2"></div>
                                            <div className="grid__col-3">Status</div>
                                            <div className="grid__col-3">Date last modified</div>
                                            <div className="grid__col-2">Edition</div>
                                            <div className="grid__col-1">Version</div>
                                        </div>
