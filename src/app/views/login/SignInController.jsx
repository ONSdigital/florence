    ...
    requestLogin(credentials) {
        this.postLoginCredentials(credentials)
            ...
            .catch(error => {
                let stateToSet = {};
                if (error) {
                    stateToSet = this.handleLoginError(error);
                }
            ...
            error.errors.forEach(errorID => {
                switch (errorID) {
                    ...
                    case "NotAuthorizedException: Incorrect username or password.":
                        errorsForBody.push(<p>Email address or password are incorrect</p>);
                        break;
                    case "NotAuthorizedException: Password attempts exceeded":
                        errorsForBody.push(<p>You've tried to sign in to your account too many times. Please try again later.</p>);
                        break;
                }
            });
            validationErrors.body = errorsForBody;
            stateToSet = {
                ...this.state,
                validationErrors
            };
            stateToSet.email.errorMsg = emailMessage;
            stateToSet.password.errorMsg = passwordMessage;
        } else {
