...
const TeamsList = props => {
    ...
    const formatDateString = (date) => {
        ...
        return <>Created <span className="font-weight--600">{completeFormattedDate}</span></>
    }
    ...
    if (previewTeams.length > 0) {
        ...
        rowsToDisplay = (filterTerm === "") ? allRowData : allRowData.filter(
            team => team.id.toLowerCase().search(filterTerm.toLowerCase()) !== -1 || team.title.toLowerCase().search(filterTerm.toLowerCase()) !== -1);
    }
        ...
        <div className="grid grid--justify-space-around">
            <div className="grid__col-9">
                ...
                <span className="margin-top--1">
                    <h2 className="inline-block">Preview teams</h2> <Link to={url.resolve("./groups/create")}> Create a new team </Link></span>
                <div className="grid__col-6">
                    <Input id="search-content-types" placeholder="Search teams by name or ID"
                           onChange={(e) => {
                               e.preventDefault();
                               setFilterTerm(e.target.value);
                           }}/>
                </div>
                <SimpleSelectableList rows={rowsToDisplay}
                                      showLoadingState={isFetchingTeams}/>
            </div>
        </div>
    );
};
