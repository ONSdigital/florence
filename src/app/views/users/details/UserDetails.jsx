...
export class UserDetails extends Component {
    ...
    renderRole() {
        if (this.props.role === "ADMIN") {
            return (
                <p id="user-role" className="margin-bottom--1">
                    {this.props.name} is an <strong>admin</strong>
                </p>
            );
        }
        ...
        if (this.props.role === "EDITOR") {
            return (
                <p id="user-role" className="margin-bottom--1">
                    {this.props.name} is a <strong>publisher</strong>
                </p>
            );
        }
        ...
        if (this.props.role === "VIEWER") {
            return (
                <p id="user-role" className="margin-bottom--1">
                    {this.props.name} is a <strong>viewer</strong>
                </p>
            );
        }
        ...
        if (!this.props.role) {
            ...
            return (
                <p id="user-role" className="margin-bottom--1">
                    {this.props.name} has no permissions
                </p>
            );
        }
        ...
        return (
            <p id="user-role" className="margin-bottom--1">
                {this.props.name} is a <strong>{this.props.role}</strong> (unrecognised user type)
            </p>
        );
    }
                ...
                <div className="drawer__heading">
                    <div className="grid grid--justify-space-around">
                        <div className="grid__col-8 margin-top--3 margin-bottom--2">
                            <h2 id="user-name">{this.props.isLoading ? "Loading user..." : this.props.name || "[No name available]"}</h2>
                    ...
                    <div className="drawer__banner">
                        <div className="grid grid--justify-space-around">
                            <div className="grid__col-8 grid--align-start margin-top--1 margin-bottom--1">
                                <Link className="btn btn--primary" to={`${location.pathname}/change-password`}>
                                    Change password
                                </Link>
                            </div>
                        </div>
                    ...
                    <div className="grid grid--justify-space-around">
                        <div className="grid__col-8 margin-top--1">
                            {this.props.isLoading ? (
                                ...
                                <div>
                                    {this.props.errorFetchingUserDetails && <p className="margin-bottom--1">Unable to show user's details</p>}
                                    {this.props.errorFetchingUserPermissions && <p className="margin-bottom--1">Unable to show user's permissions</p>}
                                    {this.props.role && this.renderRole()}
                                    {this.props.hasTemporaryPassword && (
                                        <p id="user-temporary-password" className="margin-bottom--1">
                    ...
                    <div className="drawer__footer">
                        <div className="grid grid--justify-space-around">
                            <div className="grid__col-8 margin-top--1 margin-bottom--1">
                                <div>
                                    <Link id="user-delete" className="btn btn--warning" to={`${location.pathname}/confirm-delete`}>
                                        Delete
                                    </Link>
                                    <button id="user-close" className="btn margin-left--1" onClick={this.props.onClose}>
                                        Close
                                    </button>
                                </div>
                            </div>
