...
export class UserDetails extends Component {
    ...
    renderRole() {
        ...
        if (!this.props.role) {
            log.event(`Attempt to render role for user but none passed in as prop`, log.warn(), log.data({user: this.props.email}));
            return (
                <p id="user-role" className="margin-bottom--1">{this.props.name} has no permissions</p>
            )
        }

        log.event("Attempt to render an unknown user role", log.warn(), log.data({user: this.props.email, role: this.props.role}));
        return (
