/**
 * Manage files associated with datasets
 * save RELATIVE path
 *
 * @param collectionId
 * @param data
...
function addSupplementaryFile(collectionId, data, field, idField) {
  var list = data[field];
  var downloadExtensions;
  var dataTemplate = {list: list, idField: idField};
  var html = templates.editorSupplementaryFiles(dataTemplate);
    ...
    $(data[field]).each(function (index) {
      ...
      $('#' + idField + '-delete_' + index).click(function () {
        ...
        if (result === true) {
          ...
          var fileToDelete = data.uri + data[field][index].file
          Florence.globalVars.pagePos = position;
          $(this).parent().remove();
          $.ajax({
            url: "/zebedee/content/" + collectionId + "?uri=" + fileToDelete,
            type: "DELETE",
